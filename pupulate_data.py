...
def populate():
    ...
    for spec in specialties:
        try:
            db.execute("INSERT INTO specialties (name) VALUES (?)", spec)
        except:
            pass
    ...
    doctors_data = [
        ("1755555551", "dr_smith", "John", "Smith", 2, "08:00", "16:00"), # Ortodoncia
        ("1755555552", "dra_garcia", "Elena", "García", 3, "08:00", "16:00"), # Endodoncia
        ("1755555553", "dr_perez", "Carlos", "Pérez", 4, "09:00", "17:00"), # Cirugía
        ("1755555554", "dra_lopez", "Ana", "López", 5, "09:00", "17:00")   # Odontopediatría
    ]

    print("Insertando doctores...")
    doctor_ids = []
    for dni, user, fn, ln, spec_id, start_time, end_time in doctors_data:
        try:
            id = db.execute("""
                INSERT INTO users (dni, username, hash, first_name, last_name, role_id, specialty_id, start_time, end_time)
                VALUES (?, ?, ?, ?, ?, 3, ?, ?, ?)
            """, dni, user, generate_password_hash("doctor123"), fn, ln, spec_id, start_time, end_time)
    ...
    patient_ids = []
    for dni, fn, ln in patients_data:
        try:
            id = db.execute("INSERT INTO patients (dni, first_name, last_name) VALUES (?, ?, ?)", dni, fn, ln)
            patient_ids.append(id)
        except:
            res = db.execute("SELECT patient_id FROM patients WHERE dni = ?", dni)
    ...
    appointments_list = [
        (0, 0, 1, "09:00", 45.0), # Mañana
        (1, 0, 1, "10:30", 50.0),
        (2, 1, 2, "14:00", 120.0), # Pasado mañana
        (3, 1, 2, "15:30", 35.0),
        (4, 2, 3, "08:00", 200.0),
        (0, 2, 3, "11:00", 60.0),
        (1, 3, 4, "09:45", 40.0),
